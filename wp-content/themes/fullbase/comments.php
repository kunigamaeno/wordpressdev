...
if (isset($_SERVER['SCRIPT_FILENAME']) && 'comments.php' == basename($_SERVER['SCRIPT_FILENAME'])){
    die (esc_html__('Please do not load this page directly. Thanks!', 'fullbase' )); }
if ( post_password_required() ) { ?>
    <p class="nocomments"><?php esc_html_e('This post is password protected. Enter the password to view comments.', 'fullbase'); ?></p>
<?php
    return; }
...
if ( have_comments() ) : ?>
    <h3 id="comments"><?php comments_number( esc_html__( 'No Responses', 'fullbase' ), esc_html__( '1 Response', 'fullbase' ), esc_html__( '% Responses', 'fullbase') ); ?></h3>
    ...
    if ( ! comments_open() ) : // There are comments but comments are now closed ?>
    	<p class="nocomments"><?php esc_html_e('Comments are closed.', 'fullbase'); ?></p>
    <?php
    endif;
...
comment_form(array(
  ...
  'label_submit' => ''.esc_html__('Submit Comment','fullbase').'',
  'comment_notes_after' => ''
  ));

// RSS comments link
echo '<div class="comments_rss">';
post_comments_feed_link(''.esc_html__('Comments RSS Feed', 'fullbase').'');
echo '</div>';
