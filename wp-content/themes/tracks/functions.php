...
{
 //###########kunigamaeno default image in 
 $def_img_id =get_def_img_id();

			if ( ( is_archive() || is_home() ) && $premium_layout != 'full-width' && $premium_layout != 'full-width-images' ) {
				$image = wp_get_attachment_image_src( $def_img_id , 'large' );
			} else {
				$image = wp_get_attachment_image_src( $def_img_id , 'single-post-thumbnail' );
			}

			$image     = $image[0];
			$has_image = true;

 //###########kunigamaeno default image end 
}
...
function save_default_thumbnail( $post_id ) {
 $def_img_id =get_def_img_id();//369;
	$post_thumbnail = get_post_meta( $post_id, $key = '_thumbnail_id', $single = true );
	if ( !wp_is_post_revision( $post_id ) ) {
		if ( empty( $post_thumbnail ) ) {
			update_post_meta( $post_id, $meta_key = '_thumbnail_id', $meta_value = $def_img_id );
		}
	}
}
//